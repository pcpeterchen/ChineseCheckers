...
class Board extends Component {
  ...
  restartGame = () => {
    ...
    // the _initial in _initial object is empty {}, so reassign it
    restore({ ..._initial, _initial })
  }

  render() {
    const { G, moves, endTurn, ctx, restore } = this.props
    ...
    return (
      <div className="w-75 mw7">
        ...
        >
          { // show winner and restart option when game ends
            ...
            <g>
              ...
              <rect 
                x="3" y="4" rx=".4" width="5" height="2"
                stroke="black" fill="Bisque" strokeWidth="0.1"
              />
              <text
                className="pointer"
                onClick={this.restartGame}
                x="3.8" y="5.3" style={{ fontSize: '1px' }}
              >
                restart
              </text>
            </g>
        ...
        <span className='dib ma3'>Click to pick a piece & move. Right click to cancle selection.</span>
        <span>
          <span 
